...
export default function Table(props){
  ...
  const rows = properties.map((row, index)=>{
      return (
        row.maintenanceRequests.map((request, index2) =>{
            
            request!=null ? totalRequests=totalRequests+1 : totalRequests = totalRequests + 0;
...
//   console.log(rows)
    ...
    return(
      ...
      <table>
        <thead>
          <tr>
            <th colSpan={"2"} style={{fontSize: "40px", color:"blue"}}>{"Maintenance"}</th>
            <th colSpan={"1"} style={{fontSize: "16px"}, {textAlign: "center"}}><span style={{color: "blue", fontSize: '40px'}}>{totalRequests}</span>{"    Issues"}</th>
            <th colSpan={"3"} style={{fontSize: "16px"}, {textAlign: "center"}}><span style={{color: "Gold", fontSize: '40px'}}>{numOverDays}</span>{" Over 30 Days"}</th>
            <th colSpan={"3"} style={{fontSize: "16px"}, {textAlign: "center"}}><span style={{color: "red", fontSize: '40px'}}>{}</span>{"Important"}</th>
            <th colSpan={"3"} style={{fontSize: "16px"}, {textAlign: "center"}}><span style={{color: "red", fontSize: '40px'}}>{totalUrgent}</span>{"  Urgent"}</th>
          </tr>
          <tr>
